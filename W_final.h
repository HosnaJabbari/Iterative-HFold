...
class W_final: public s_min_folding{
    ...
    protected:
		...
		void backtrack_restricted_pmo (seq_interval *cur_interval, str_features *fres);
        // backtrack, the restricted case
		...
		void backtrack_restricted_pkonly_pmo (seq_interval *cur_interval, str_features *fres);
		// backtrack, the restricted case with pk only base pairs
	
        void compute_W_restricted (int j, str_features *fres);
        // fill the W array, the restricted case

		void compute_W_restricted_pmo (int j, str_features *fres);
        // fill the W array, the restricted case
		...
		void compute_W_restricted_pkonly_pmo(int j, str_features *fres);

        int compute_W_br2_restricted (int j, str_features *fres, int &must_choose_this_branch);

		int compute_W_br2_restricted_pmo (int j, str_features *fres, int &must_choose_this_branch);
	
		// Hosna, April 3, 2012
		int compute_W_br2_restricted_pkonly (int j, str_features *fres, int &must_choose_this_branch);

		int compute_W_br2_restricted_pkonly_pmo (int j, str_features *fres, int &must_choose_this_branch);		
